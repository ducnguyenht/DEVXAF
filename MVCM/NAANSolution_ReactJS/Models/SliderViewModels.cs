...
namespace NAANSolution_ReactJS.Models
{
    public class SliderViewModels
    {
        ...
        public static IList<SliderViewModels> MapFromBOs(IList<Image_BO> bos)
        {
            IList<SliderViewModels> apiObjs = new List<SliderViewModels>();
            foreach (var bo in bos)
            {
                SliderViewModels apiObj = new SliderViewModels();
                apiObj.MapFromBO(bo);
                apiObjs.Add(apiObj);
            }
            return apiObjs;
